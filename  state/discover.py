...
def log(m): print(f"[discover] {m}", flush=True)
...
def save_groups(groups):
    with open("groups.txt", "w") as f:
        for g in sorted(groups): f.write(g + "\n")

async def search_groups(page, keyword):
    found = []
    try:
        url = f"https://www.facebook.com/search/groups/?q={keyword.replace(' ', '%20')}"
        ...
        for l in links[:50]:
            try:
                ...
                if len(found) >= MAX_PER_KEYWORD: break
            except: continue
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            for kw in keywords[:60]:
                if len(all_groups) >= MAX_TOTAL: break
